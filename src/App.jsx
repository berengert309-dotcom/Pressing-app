...
function buildTicketHtml(order, unitPrices = {}) {
  const nTags = totalGarments(order);
  const rows = (order.itemsList && order.itemsList.length) ? order.itemsList.filter(r => r.qty > 0) : null;
  ...
  const rowsHtml = rows
    ? rows.map((r) => {
        ...
        return `<tr style="border-bottom:1px dotted #999">
          <td style="padding:0.2rem 0;font-size:0.75rem">${escapeHtml(r.type)}</td>
          <td style="padding:0.2rem 0;text-align:center;font-size:0.75rem">${r.qty}</td>
          <td style="padding:0.2rem 0;text-align:right;font-size:0.75rem">${unit ? escapeHtml(money(lineTotal)) : "—"}</td>
        </tr>`;
      }).join("")
    : `<tr><td colspan="3" style="padding:0.2rem 0;font-size:0.75rem">${escapeHtml(order.items)}</td></tr>`;

  const tagsHtml = Array.from({ length: nTags }).map(() =>
    `<div style="border:1px dashed #000;border-radius:5px;padding:0.25rem 0.4rem;font-weight:700;font-size:0.85rem;min-width:56px;text-align:center;display:inline-block;margin:2px">${escapeHtml(labelCode(order))}</div>`
  ).join("");
...
<head>
<meta charset="UTF-8" />
<title>Ticket ${escapeHtml(ticketNumber(order))}</title>
<style>
  * { box-sizing: border-box; }
  body { margin:0; padding:1rem; font-family:'SF Mono','Roboto Mono',monospace; color:#000; background:#fff; font-size:0.78rem; line-height:1.5; width:280px; }
  table { width:100%; border-collapse: collapse; margin-top:0.2rem; }
</style>
</head>
<body>
  <div style="text-align:center;font-weight:800;font-size:1.05rem;margin-bottom:0.1rem">PRESSING LA MAIN DE DIEU</div>
  <div style="text-align:center;font-size:0.7rem;margin-bottom:0.6rem">Ticket de d&eacute;p&ocirc;t</div>
  ${order.isExpress ? `<div style="text-align:center;font-weight:800;font-size:0.8rem;background:#000;color:#fff;padding:0.2rem 0;margin-bottom:0.3rem">⚡ COMMANDE EXPRESS</div>` : ""}
  <div style="border-top:1px dashed #000;margin:0.4rem 0"></div>
  <div style="text-align:center;font-weight:800;font-size:1.6rem;letter-spacing:0.03em;margin:0.3rem 0">${escapeHtml(labelCode(order))}</div>
  <div style="text-align:center;font-size:0.65rem;color:#333;margin-bottom:0.3rem">N&deg; d'&eacute;tiquetage — &agrave; noter sur chaque v&ecirc;tement</div>
  <div style="border-top:1px dashed #000;margin:0.4rem 0"></div>
  <div>N&deg; ticket : ${escapeHtml(ticketNumber(order))}</div>
  <div>Client : ${escapeHtml(order.client)}</div>
  ${order.phone ? `<div>T&eacute;l&eacute;phone : ${escapeHtml(order.phone)}</div>` : ""}
  <div>D&eacute;pos&eacute; le : ${escapeHtml(fmtDate(order.depositDate))}${fmtTime(order.depositDate) ? ` &agrave; ${escapeHtml(fmtTime(order.depositDate))}` : ""}</div>
  ${order.dueDate ? `<div>Retrait pr&eacute;vu : ${escapeHtml(fmtDate(order.dueDate))}${order.dueTime ? ` &agrave; ${escapeHtml(order.dueTime)}` : ""}</div>` : ""}
  ${order.notes ? `<div style="margin-top:0.2rem">Observations : ${escapeHtml(order.notes)}</div>` : ""}
  <div style="border-top:1px dashed #000;margin:0.4rem 0"></div>
  <table>
    <thead>
      <tr style="border-bottom:1px solid #000">
        <th style="text-align:left;padding:0.15rem 0;font-size:0.72rem">Article</th>
        <th style="text-align:center;padding:0.15rem 0;font-size:0.72rem">Qt&eacute;</th>
        <th style="text-align:right;padding:0.15rem 0;font-size:0.72rem">Prix</th>
      </tr>
    </thead>
  ...
  <div style="display:flex;justify-content:space-between;font-weight:700;font-size:0.95rem">
    <span>Total</span><span>${displayTotal ? escapeHtml(money(displayTotal)) : "—"}</span>
  </div>
  <div style="border-top:1px dashed #000;margin:0.4rem 0"></div>
  <div style="text-align:center;font-size:0.68rem;margin-top:0.4rem">Merci de pr&eacute;senter ce ticket au retrait.</div>
  ...
  <div style="border-top:2px dashed #000;margin:0.8rem 0 0.4rem"></div>
  <div style="text-align:center;font-size:0.62rem;color:#333;margin-bottom:0.3rem">&#9986;&#65039; D&eacute;couper les &eacute;tiquettes ci-dessous, une par v&ecirc;tement</div>
...
export default function App() {
  ...
  const printTicket = (order) => {
    const html = buildTicketHtml(order, unitPrices);
    const w = window.open("", "_blank");
    if (!w) {
      window.alert("Ton navigateur a bloqué l'ouverture de la fenêtre d'impression. Autorise les pop-ups pour ce site et réessaie.");
      return;
    }
    w.document.open();
  ...
  // (impression gérée directement par printTicket(order), plus besoin de printOrder ici)
        ...
        <div style={{ padding: "1.1rem 1.1rem 0" }}>
          ...
          {!loaded ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.7rem" }}>
              {filtered.map(order => (
                <OrderCard key={order.id} order={order} onAdvance={() => advanceStatus(order.id)} onPrint={() => printTicket(order)} />
              ))}
            </div>
          )}
        </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
        {client.orders
          .sort((a, b) => b.depositDate.localeCompare(a.depositDate))
          .map(o => <OrderCard key={o.id} order={o} onAdvance={() => onAdvance(o.id)} onPrint={() => onPrint(o)} />)}
      </div>
    </div>
  );
}
